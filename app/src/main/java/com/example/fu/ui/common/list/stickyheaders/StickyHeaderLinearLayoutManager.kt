 ...
 *
 *
 * Modifications copyright 2020 Redmadrobot SPb
 *
 * Changes made to the original file:
 * - Moved StickyHeaderCallback in as a nested interface
 * - Allow use with any RecyclerView.Adapter that implements the StickyHeaderCallbacks
 * - Throw if the StickyHeaderCallbacks is not implemented by the adapter
 * - Constructor to add from layout
 * - Respect margins when choosing anchor view (only for vertical orientation)
 * - Small refactoring
 */
 ...
 * The adapter must override [StickyHeaderCallbacks.isStickyHeader] to
...
class StickyHeaderLinearLayoutManager : LinearLayoutManager  {
    ...
    override fun onAdapterChanged(oldAdapter: RecyclerView.Adapter<*>?, newAdapter: RecyclerView.Adapter<*>?) {
        require (newAdapter is StickyHeaderCallbacks) {
            "Adapter must implement StickyHeaderCallbacks."
        }
    ...
    private fun createStickyHeader(recycler: RecyclerView.Recycler, position: Int) {
        ...
        (adapter as StickyHeaderCallbacks?)?.setupStickyHeaderView(stickyHeader)
    ...
    private fun scrapStickyHeader(recycler: RecyclerView.Recycler?) {
        ...
        (adapter as StickyHeaderCallbacks?)?.teardownStickyHeaderView(stickyHeader)
    ...
    private fun isViewValidAnchor(view: View, params: RecyclerView.LayoutParams): Boolean {
        val bottomMargin = (view.layoutParams as? ViewGroup.MarginLayoutParams)?.bottomMargin ?: 0
        ...
        return when {
            !params.isItemRemoved && !params.isViewInvalid -> when (orientation) {
                VERTICAL -> when {
                    reverseLayout -> view.top - topMargin + view.translationY <= height + translationY
                    else -> view.bottom + bottomMargin - view.translationY >= translationY
                }
                // Maybe horizontal orientation does not respect margins
                else -> when {
    ...
    private inner class HeaderPositionsAdapterDataObserver : RecyclerView.AdapterDataObserver() {
        ...
        private fun isItemStickyHeader(position: Int): Boolean {
            return (adapter as StickyHeaderCallbacks?)?.isStickyHeader(position) ?: false
        }
